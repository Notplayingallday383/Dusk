...
// dist/cli/dpx.js
var dpx_exports = {};
__export(dpx_exports, {
  main: () => main
});
module.exports = __toCommonJS(dpx_exports);
...
var findLocalBin = (cwd, name) => {
  const bin = path9.join(cwd, "node_modules", ".bin", name);
  if (fs7.existsSync(bin))
    return bin;
  let dir = cwd;
  while (dir !== "/") {
    const candidate = path9.join(dir, "node_modules", ".bin", name);
...
var execCommand = async (opts) => {
  ...
  if (!binPath && opts.tempInstall) {
    const tmpDir = path9.join("/tmp", `dpx-${Date.now()}`);
  ...
  const result = child_process2.spawnSync(binPath, opts.args, {
    cwd: opts.cwd,
    stdio: "inherit",
    env: process.env
  });
