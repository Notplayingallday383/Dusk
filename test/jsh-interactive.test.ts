// Interactive `/bin/jsh` — the mode the demo uses.
...
test('jsh interactive: prompt appears on boot', async () => {
  const j = await startJsh();
  expect(await j.waitFor('jsh$ ')).toBe(true);
  await j.cleanup();
}, 60_000);

test('jsh interactive: echo hello via stdin', async () => {
  const j = await startJsh();
  await j.waitFor('jsh$ ');
...
test('jsh interactive: multiple commands in sequence', async () => {
  const j = await startJsh();
  await j.waitFor('jsh$ ');
...
test('jsh interactive: variable persistence across prompts', async () => {
  const j = await startJsh();
  await j.waitFor('jsh$ ');
  await j.send('x=42');
  await j.waitFor('jsh$ '); // wait for next prompt
  ...
  // The current jsh main.ts's runOnce creates one Bash per script — vars
...
test('jsh interactive: pipeline works', async () => {
  const j = await startJsh();
  await j.waitFor('jsh$ ');
...
test('jsh interactive: reads DuskJS-seeded /etc/hostname', async () => {
  const j = await startJsh();
  await j.waitFor('jsh$ ');
  ...
  const promptCount = (buf.match(/jsh\$ /g) ?? []).length;
  expect(promptCount).toBeGreaterThanOrEqual(2);
  await j.cleanup();
}, 60_000);

test('jsh interactive: exit terminates the shell', async () => {
  const j = await startJsh();
  await j.waitFor('jsh$ ');
  await j.send('exit');
...
// ─── Node REPL mode ──────────────────────────────────────────────────────
// Typing `node` at the jsh prompt enters an interactive JS REPL that keeps
// its own persistent context across lines. `.exit` returns to jsh.

test('jsh interactive: node REPL enters and evaluates expressions', async () => {
  const j = await startJsh();
  await j.waitFor('jsh$ ');
  await j.send('node');
  ...
  await j.send('.exit');
  // After .exit we should get another jsh$ prompt.
  await new Promise((r) => setTimeout(r, 300));
  await j.send('echo back-in-jsh');
  const gotJsh = await j.waitFor('back-in-jsh');
  expect(gotJsh).toBe(true);
  await j.cleanup();
}, 60_000);

test('jsh interactive: node REPL persists variables across lines', async () => {
  const j = await startJsh();
  await j.waitFor('jsh$ ');
  await j.send('node');
  await j.waitFor('> ');
...
test('jsh interactive: node REPL pretty-prints objects and arrays', async () => {
  const j = await startJsh();
  await j.waitFor('jsh$ ');
  await j.send('node');
  await j.waitFor('> ');
...
test('jsh interactive: node REPL .clear resets context', async () => {
  const j = await startJsh();
  await j.waitFor('jsh$ ');
  await j.send('node');
  await j.waitFor('> ');
