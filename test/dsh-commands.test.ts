// Comprehensive command coverage for /bin/dsh (vendored just-bash).
// Every test uses the same call shape the demo uses:
//   pm.spawnSync('/bin/dsh', ['-c', script], { cwd: '/', stdin? })
...
// Run a dsh -c script and return {stdout, stderr, status}.
const dsh = async (script: string, stdin?: string): Promise<{ stdout: string; stderr: string; status: number }> => {
  const out: string[] = [];
...
test('redirect > writes file', async () => {
  const r = await dsh('echo hello > /tmp/x && cat /tmp/x');
  expect(r.status).toBe(0);
  expect(r.stdout).toBe('hello\n');
});

test('redirect >> appends', async () => {
  const r = await dsh('echo a > /tmp/log; echo b >> /tmp/log; cat /tmp/log');
  expect(r.status).toBe(0);
  expect(r.stdout).toBe('a\nb\n');
});

test('redirect < reads', async () => {
  const r = await dsh('echo input > /tmp/in; cat < /tmp/in');
  expect(r.status).toBe(0);
  expect(r.stdout).toBe('input\n');
});

test('mkdir + ls', async () => {
  const r = await dsh('mkdir -p /tmp/d/e/f && ls /tmp/d');
  expect(r.status).toBe(0);
  expect(r.stdout.trim()).toBe('e');
});

test('mv renames a file', async () => {
  const r = await dsh('echo hi > /tmp/a; mv /tmp/a /tmp/b; cat /tmp/b');
  expect(r.status).toBe(0);
  expect(r.stdout).toBe('hi\n');
});

test('cp copies a file', async () => {
  const r = await dsh('echo hi > /tmp/a; cp /tmp/a /tmp/b; cat /tmp/b');
  expect(r.status).toBe(0);
  expect(r.stdout).toBe('hi\n');
});

test('rm removes a file', async () => {
  const r = await dsh('echo hi > /tmp/a; rm /tmp/a; if [ -f /tmp/a ]; then echo still-there; else echo gone; fi');
  expect(r.status).toBe(0);
  expect(r.stdout).toBe('gone\n');
});

test('touch creates empty file', async () => {
  const r = await dsh('touch /tmp/empty && ls -la /tmp | grep empty | head -1');
  expect(r.status).toBe(0);
...
test('grep: basic pattern match', async () => {
  const r = await dsh('printf "apple\\nbanana\\napricot\\n" | grep ^a');
  expect(r.status).toBe(0);
...
test('awk: print field', async () => {
  const r = await dsh('echo "a b c" | awk "{print \\$2}"');
  expect(r.status).toBe(0);
  expect(r.stdout).toBe('b\n');
});

test('awk: sum column', async () => {
  const r = await dsh('printf "1\\n2\\n3\\n4\\n" | awk "{s+=\\$1} END {print s}"');
  expect(r.status).toBe(0);
...
test('awk: -F custom separator', async () => {
  const r = await dsh('echo "a,b,c" | awk -F, "{print \\$2}"');
  expect(r.status).toBe(0);
  expect(r.stdout).toBe('b\n');
});
...
test('cut -f fields', async () => {
  const r = await dsh('echo "a,b,c,d" | cut -d, -f2,4');
  expect(r.status).toBe(0);
  expect(r.stdout).toBe('b,d\n');
});

test('cut -c characters', async () => {
  const r = await dsh('echo hello | cut -c1-3');
  expect(r.status).toBe(0);
...
test('wc counts lines/words/chars', async () => {
  const r = await dsh('printf "one two\\nthree\\n" | wc');
  expect(r.status).toBe(0);
...
test('wc -w words only', async () => {
  const r = await dsh('echo "one two three" | wc -w');
  expect(r.status).toBe(0);
  expect(r.stdout.trim()).toBe('3');
});
...
test('tr: char substitution', async () => {
  const r = await dsh('echo abc | tr a-c x-z');
  expect(r.status).toBe(0);
  expect(r.stdout).toBe('xyz\n');
});

test('tr -d delete chars', async () => {
  const r = await dsh('echo "a b c" | tr -d " "');
  expect(r.status).toBe(0);
...
test('printf: %s format', async () => {
  const r = await dsh('printf "%s\\n" hello');
  expect(r.status).toBe(0);
  expect(r.stdout).toBe('hello\n');
});

test('printf: %d format', async () => {
  const r = await dsh('printf "%d\\n" 42');
  expect(r.status).toBe(0);
  expect(r.stdout).toBe('42\n');
});

test('printf: multiple args', async () => {
  const r = await dsh('printf "%s=%d\\n" x 5');
  expect(r.status).toBe(0);
...
test('jq: select field', async () => {
  const r = await dsh('printf \'{"name":"dusk"}\' | jq .name');
  expect(r.status).toBe(0);
  expect(r.stdout.trim()).toBe('"dusk"');
});

test('jq: -r raw output', async () => {
  const r = await dsh('printf \'{"name":"dusk"}\' | jq -r .name');
  expect(r.status).toBe(0);
  expect(r.stdout).toBe('dusk\n');
});

test('jq: array indexing', async () => {
  const r = await dsh('printf \'[10,20,30]\' | jq .[1]');
  expect(r.status).toBe(0);
...
test('base64: encode', async () => {
  const r = await dsh('echo -n hello | base64');
  expect(r.status).toBe(0);
  expect(r.stdout.trim()).toBe('aGVsbG8=');
});

test('base64 -d: decode', async () => {
  const r = await dsh('echo -n aGVsbG8= | base64 -d');
  expect(r.status).toBe(0);
  expect(r.stdout).toBe('hello');
});
...
test('basename', async () => {
  const r = await dsh('basename /path/to/file.txt');
  expect(r.status).toBe(0);
  expect(r.stdout).toBe('file.txt\n');
});

test('dirname', async () => {
  const r = await dsh('dirname /path/to/file.txt');
  expect(r.status).toBe(0);
...
test('find lists files', async () => {
  const r = await dsh('mkdir -p /tmp/f && touch /tmp/f/a /tmp/f/b && find /tmp/f -type f | sort');
  expect(r.status).toBe(0);
  expect(r.stdout).toBe('/tmp/f/a\n/tmp/f/b\n');
});

test('find -name glob', async () => {
  const r = await dsh('mkdir -p /tmp/g && touch /tmp/g/one.txt /tmp/g/two.log && find /tmp/g -name "*.txt"');
  expect(r.status).toBe(0);
...
// xargs deadlocks in the current dsh integration — it uses ctx.exec to run
...
test('js-exec: simple JS math', async () => {
  const r = await dsh('js-exec -c "console.log(2 + 2)"');
  expect(r.status).toBe(0);
  expect(r.stdout.trim()).toBe('4');
});

test('js-exec: uses node crypto', async () => {
  const r = await dsh('js-exec -c "console.log(require(\\"node:crypto\\").randomUUID().length)"');
  expect(r.status).toBe(0);
...
test('reads seeded /etc/hostname from DuskJS fs', async () => {
  const r = await dsh('cat /etc/hostname');
  expect(r.status).toBe(0);
  expect(r.stdout.trim().length).toBeGreaterThan(0);
});

test('reads /etc/passwd from DuskJS fs', async () => {
  const r = await dsh('grep root /etc/passwd');
  expect(r.status).toBe(0);
...
test('dsh --version prints a version', async () => {
  const r = await dsh('--version 2>&1 || dsh --version 2>&1 || echo missing');
  // Either the -c script mode variant works, or the direct --version.
  expect(r.stdout.length + r.stderr.length).toBeGreaterThan(0);
});

// ─── TFS persistence ─────────────────────────────────────────────────────
// dsh backs writes with DuskJS TFS via TfsFs (not an in-memory snapshot),
// so files written by one dsh invocation must be visible to the next, and
// to non-dsh binaries that read TFS directly.

test('writes persist across dsh invocations (TFS-backed)', async () => {
  const out: string[] = [];
  const repl = await bootRepl((t) => out.push(t), { fs: 'memory' });
  try {
    const write = await repl.processManager.spawnSync('/bin/dsh',
      ['-c', 'echo persistent-marker > /tmp/tfs-test.txt'], { cwd: '/' });
    expect(write.status).toBe(0);

    const read = await repl.processManager.spawnSync('/bin/dsh',
...
test('dsh writes are visible to /bin/cat (cross-binary TFS)', async () => {
  const out: string[] = [];
  const repl = await bootRepl((t) => out.push(t), { fs: 'memory' });
  try {
...
test('mkdir -p in dsh creates directories visible via /bin/ls', async () => {
  const out: string[] = [];
  const repl = await bootRepl((t) => out.push(t), { fs: 'memory' });
  try {
    const m = await repl.processManager.spawnSync('/bin/dsh',
